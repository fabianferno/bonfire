import type { ChannelAdapter, InboundMessage, TenantPayload } from './base.js';
import { EventEmitter } from 'node:events';

export interface WebChatBus {
  enqueue(userId: string, text: string, tenant?: string, envOverride?: Record<string, string>, tenantInline?: TenantPayload): Promise<string>;
...
export class WebChatAdapter implements ChannelAdapter, WebChatBus {
  ...
  async enqueue(userId: string, text: string, tenant?: string, envOverride?: Record<string, string>, tenantInline?: TenantPayload): Promise<string> {
